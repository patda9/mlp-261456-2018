...
def k_fold(activations, form, input, d, k):
    ...
    for i in range(k):
        ...
        d_train = output_temp
        layers = train(activations, form, training_set[i], d_train[i])
...
def train(activations, form, training_set, d_train):
...
form = [input.shape[1], 6, 4, 1]
k_fold(activations, form, input, d, 10)